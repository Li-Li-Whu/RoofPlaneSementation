...
//#pragma omp parallel for
	for (int i = 0; i<pointNum; ++i)
	{
		...
		pcaInfos[i].idxDist.resize(k);
		for (int j = 0; j < k; ++j)
		{
			pcaInfos[i].idxDist[j] = dis_temp[j];
		}
...
//#pragma omp parallel for
	for (int i = 0; i < pointNum; ++i)
	{
		...
		pcaInfos[i].planePt.val[0] = h_mean_x; pcaInfos[i].planePt.val[1] = h_mean_y; pcaInfos[i].planePt.val[2] = h_mean_z;
